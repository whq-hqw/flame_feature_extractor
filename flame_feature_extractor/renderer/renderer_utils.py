...
class RenderMesh(nn.Module, ModuleUtilsMixin):
    ...
    def _build_cameras(self, transform_matrix, focal_length):
        ...
        screen_size = (
            torch.tensor([self.image_size, self.image_size], device=self.device).float()[None].repeat(batch_size, 1)
        )
        cameras_kwargs = {
...
class PointRenderer(nn.Module):
    def __init__(self, image_size=256, device="cpu"):
        ...
        raster_settings = PointsRasterizationSettings(image_size=image_size, radius=0.005, points_per_pixel=10)
    ...
    def forward(self, points, D=3, E=15, A=30, coords=True, ex_points=None):
        ...
        if coords:
            ...
            verts = torch.cat([verts, cod.unsqueeze(0).expand(verts.shape[0], -1, -1)], dim=1)
...
class TextureRenderer(nn.Module):
    ...
    def _build_cameras(self, transform_matrix, focal_length, principal_point, image_size):
        batch_size = transform_matrix.shape[0]
        screen_size = torch.tensor([image_size, image_size], device=self.device).float()[None].repeat(batch_size, 1)
        cameras_kwargs = {
    ...
    def forward(
        ...
        raster_settings = RasterizationSettings(
            image_size=image_size, blur_radius=0.0, faces_per_pixel=1, perspective_correct=True, cull_backfaces=True
        )
        ...
        with torch.no_grad():
            if hasattr(self, "flame_mask"):
                ...
                meshes_masked = Meshes(verts=vertices_world, faces=faces[:, self.flame_mask], textures=textures_verts)

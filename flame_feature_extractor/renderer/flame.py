...
class FLAME(nn.Module):
    ...
    def forward(
        ...
        pose_params: torch.Tensor | None,
        eye_pose_params: torch.Tensor | None,
    ):
        """
        Input:
        ...
        full_pose = torch.cat(
            [pose_params[:, :3], self.neck_pose.expand(batch_size, -1), pose_params[:, 3:], eye_pose_params], dim=1  # type: ignore
        )  # type: ignore
...
class FLAMEDense(FLAME):
    def __init__(self, n_shape, n_exp, scale=1.0):
        super().__init__(n_shape, n_exp, scale)
        flame_lmk_mediapipe = self.flame_ckpt["lmk_embeddings_mediapipe"]
    ...
    def forward(self, shape_params=None, expression_params=None, pose_params=None, eye_pose_params=None):
        vertices, landmarks = super().forward(shape_params, expression_params, pose_params, eye_pose_params)
        batch_size = shape_params.shape[0]
        ...
        landmarks_mediapipe = vertices2landmarks(
            vertices, self.faces_tensor, lmk_faces_idx_mediapipe, lmk_bary_coords_mediapipe
        )
        return vertices, landmarks, landmarks_mediapipe
